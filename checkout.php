...
if(!isset($_SESSION['id'])){
    echo "<script> alert('Login to check the cart'); window.location.href='index.php'; </script>";
}
						...
						<a href="checkout.php">
							<h3> <div class="total">
								<i class="glyphicon glyphicon-shopping-cart" aria-hidden="true"></i>
                                                                <?php
                                                                    $query="select count(*) as cnt from tblbid where username='{$_SESSION['id']}'";
                                                                    $result=  mysqli_query($link,$query);
                                                                    
                                                                    $row=  mysqli_fetch_assoc($result);
                                                                ?>
								(<?php echo $row['cnt'];?> items)</div>
								
							</h3>
						</a>
						<!--<p><a href="delcart.php?del=all" class="simpleCart_empty">Empty Cart</a></p>-->
			</div>	
		   </div>
		<div class="clearfix"></div>
	</div>
...
<div class="single">
	<div class="container">
            ...
            <h3><center>My Cart</center></h3><br/>
				...
				<thead>
                    ...
                    if(isset($_GET['m'])){
                        ...
                        while($row=  mysqli_fetch_assoc($result)){
                            $query="select * from tblproducts where id='{$row['id']}'";
                            $result1=mysqli_query($link,$query);
                            $row1=mysqli_fetch_assoc($result1);
                            $fn="productimg/".$row1['filename0'];
                            $tot=$tot+($row1['price']*$row['qty']);
                    ?>
			
					<tr class="rem1">
						<td class="invert-closeb">
							<div class="rem">
								<a href="delprod.php?r=<?php echo $row1['id']; ?>"><div class="close1"> </div></a>
							</div>
						...
						<td class="invert-image"><a href="#"><img src="<?php echo $fn; ?>" width="100px;" height="100px;"/></a></td>
						<td class="invert"><?php echo $row1['name'];?></td>
						<td class="invert"> <?php echo $row1['model'];?></td>
                                                <td class="invert"> <?php echo $row['qty'];?></td>
						<td class="invert"> <?php echo $row1['price'];?></td>
                                                <td class="invert"> <?php echo $row['qty']*$row1['price'];?></td>

					</tr>
					 <?php
                                              }                                              
                                         ?>
                                        <tr><td></td><td></td><td></td><td></td><td></td><td><strong>Total</strong></td><td><strong><?php echo $tot; ?></strong></td></tr>
				</table>
		</div>
	</div>

	<div class="coupons-gd">
	<span class="glyphicon glyphicon-credit-card" aria-hidden="true"></span>
        Click on Payment Options to place order and proceed.
        <a href="process.php?type=cash&amt=<?php echo $tot; ?>" class="item_add hvr-outline-out button2">&nbsp;&nbsp;&nbsp;Cash on Delivery&nbsp;&nbsp;</a>
        <a href="process.php?type1=card&amt=<?php echo $tot; ?>" class="item_add hvr-outline-out button2">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Online&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>
    </div>		
</div>
