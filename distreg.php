...
<?php
    if(isset($_REQUEST['save'])){
        ...
        $query="select * from tbldistributor where username='{$_POST['user']}'";
        ...
        $query="insert into tbldistributor(name,mobile,email,address,username,password) values('{$_POST['name']}','{$_POST['mobile']}','{$_POST['email']}','{$_POST['address']}','{$_POST['user']}','{$_POST['pass']}')";
        $result=mysqli_query($link,$query);
        if($result){
            echo "<Script> alert('Registration request sent to adminstrator');
